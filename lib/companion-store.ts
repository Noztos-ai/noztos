  ...
  ingestClaudeEvent(event: ClaudeEvent): void {
    ...
    if (event.type === 'claude_event' && !event.payload?.event && Array.isArray(event.payload?.persistRows)) {
      ...
      for (const r of event.payload.persistRows) {
        ...
        // is a safe terminal signal: mark the chat idle so the chat
        // input unlocks AND clear workflowRunId so the WorkflowRunCard
        // auto-dismisses now that the final response is in the chat.
        // The message-grouping logic in ChatPanel then promotes this
        // row to a fully-rendered final response instead of burying it
        // as a one-line preview inside a work block.
        if (r.role === 'assistant' && typeof r.id === 'string' && r.id.endsWith('-final') && r.id.startsWith('wf-')) {
          this.detachWorkflowRun(sid)
        }
      }
      return
    }
